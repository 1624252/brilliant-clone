...
const systemPrompt = [
  ...
  '- NEVER use a JavaScript reserved word as a variable/identifier — this is a fatal compile error. In particular do NOT name object distance `do` (use objectDistance, doDist, objU, or u); also avoid in, if, for, var, class, new, case, default, etc. as names.',
  '',
  'TOOLKIT (already in scope — use it for correct physics; do not reinvent it or reference anything not listed here)',
  '- thinLens(doDist, f) -> {di, m, real, inverted}. formImage(doDist, f) -> {di, m, real, inverted, upright, type} (type "convex"/"concave"/"flat"). (doDist = object distance; NEVER name a variable `do` — it is a reserved JS keyword.)',
  ...
  '4. ACCURATE TO REALITY & CORRECT PHYSICS: the simulation must faithfully reproduce how this optics phenomenon ACTUALLY works in the real world — correct equations, correct ray directions, and correct relative magnitudes (not just something that looks plausible). Use the toolkit, not hand-rolled geometry. Single lens: principalRays (solid rays + dashed back-traces for virtual images). Multi-lens (telescope/microscope/eye): use chain, OR trace a BUNDLE of 4-6 rays and bend each at EVERY lens with the slope rule u_out = u_in - y/f. NEVER draw two straight chief lines through the lens centers.',
...
const designSystemPrompt = [
  'A learner gives you a SHORT optics TOPIC (e.g. "chromatic aberration", "prism", "convex lens", "telescope"). Act as an expert optics teacher AND interface designer and ENRICH it into a DETAILED PLAN for ONE interactive, informative React + TypeScript + SVG simulation of that topic. Return ONLY JSON: {"brief": string}, where brief is the full plan written as labeled sections separated by " | ".',
  'ALWAYS BUILD IN these concrete optics ingredients (include every one that fits the topic, and they almost always fit): light RAYS (draw them as lines/bundles and bend/refract them at each element), at least 2 interactive SLIDERS, the LENS(es) (or prism/mirror) drawn as real shapes on an optical axis, focal markers F/2F and the FOCI, a SCREEN / PROJECTION / PREVIEW PANEL that renders the actual result (the formed image, the dispersed colors, the fringing), and clear LABELS + live numeric READOUTS. Briefly note HOW it works in React (state from the sliders drives the ray geometry and the rendered result each frame). Think like the worked example below and make the plan that concrete.',
  'ACCURACY: the simulation must be ACCURATE TO HOW THIS PHENOMENON REALLY WORKS in the real world — correct physics and equations, correct directions and relative magnitudes, correct qualitative behavior, and sensible numeric ranges. Spell out the real, expected behavior explicitly so the implementation matches reality, not a vague or decorative approximation.',
  ...
  '- Scene: the objects to draw — the candle/arrow object, the lens(es)/prism as real shapes, the optical axis, focal markers F/2F, the LIGHT RAYS (always present), and a screen / retina / projection / preview panel.',
  '- Controls: 2-3 SLIDERS (always at least 2), each with a name, a numeric range, and exactly what it changes in the scene.',
